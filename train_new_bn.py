...
def inference(input_tensor, train, regularizer):
    ...
    with tf.variable_scope("layer5-conv4"):
        ...
        temp4=tf.nn.bias_add(conv4, conv4_biases)
        relu4= tf.nn.relu(batch_norm_layer(temp4,train))
    ...
    with tf.variable_scope('layer7-conv5'):
        ...
        temp5=tf.nn.bias_add(conv5, conv5_biases)
        relu5= tf.nn.relu(batch_norm_layer(temp5,train))
    with tf.variable_scope("layer8-conv6"):
        ...
        temp6=tf.nn.bias_add(conv6, conv6_biases)
        relu6= tf.nn.relu(batch_norm_layer(temp6,train))
...
#learning_rate=tf.train.exponential_decay(0.04,global_step,1000,0.9)
...
for epoch in range(n_epoch):
    ...
    for x_train_a, y_train_a in minibatches(x_train, y_train, batch_size, shuffle=True):
        _=sess.run([train_op],feed_dict={x: x_train_a, y_: y_train_a, keep_prob: 0.5,train:1})
